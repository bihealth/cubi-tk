"""Tests for ``cubi_sak.snappy.itransfer_ngs_mapping``.
...
def test_run_snappy_itransfer_ngs_mapping_smoke_test(mocker):
    ...
    for member in ("index", "father", "mother"):
        for ext in ("", ".md5"):
            fake_file_paths.append(
                "%s/ngs_mapping/work/bwa.%s-N1-DNA1-WES1/out/%s-N1-DNA1-WES1.fastq.gz%s"
                % (fake_base_path, member, member, ext)
            )
            fs.create_file(fake_file_paths[-1])
            fake_file_paths.append(
                "%s/ngs_mapping/work/bwa.%s-N1-DNA1-WES1/log/bwa.%s-N1-DNA1-WES1.log%s"
                % (fake_base_path, member, member, ext)
            )
            fs.create_file(fake_file_paths[-1])
    ...
    mocker.patch("cubi_sak.snappy.itransfer_ngs_mapping.os", fake_os)
    ...
    for path in fake_file_paths:
        mapper_index, rel_path = os.path.relpath(
            path, os.path.join(fake_base_path, "ngs_mapping/work")
        ).split("/", 1)
        _mapper, index = mapper_index.split(".", 1)
