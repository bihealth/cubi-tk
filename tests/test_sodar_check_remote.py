...
def test_filecomparisoncheck_compare_local_and_remote_files(irods_file_objects, local_file_objects):
    ...
    local_file_objects.update({test_dir_path / "test4": [FileDataObject("test4.txt", str(test_dir_path / "test4.txt"), "abcde")]})
...
def test_sodar_check_remote(mock_rsc, irods_file_objects, capsys):
    ...
    argv = [
        'sodar',
        'check-remote',
        '-p', str(pathlib.Path(__file__).resolve().parent / "data" / "sodar_check_remote"),
        'DUMMY-UUID'
    ]
    ...
    def get_expected(both=(1,2), local=(3,), remote=(3,5), incl_checksums = False):
        checksums = get_checksum()
        expected = []
        if both:
            expected += ['Files found BOTH locally and remotely:']
            for n in both:
                expected += [f'{pathlib.Path(__file__).resolve().parent}/data/sodar_check_remote/test{n}:']
                expected += [f'    test{n}.txt' + (f"  ({next(checksums)})" if incl_checksums else "")]
    ...
    output = [l for l in capsys.readouterr().out.split('\n') if l and not re.match(r'(I -|S -| \.\.\.)', l)]
