...
class DkfzMetaParser:
    ...
    column_names = {
        "FASTQ_FILE":                 [
            {"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Name", "Column_name": None}},
            {"f": "meta_to_internal", "args": "Filename"}
        ],
        "MATE":                       [{"f": "meta_to_internal", "args": "Mate"}],
        "READ":                       [{"f": "meta_to_internal", "args": "Mate"}],
        "MD5":                        [
            {"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Characteristics", "Column_name": "Checksum"}},
            {"f": "meta_to_internal", "args": "Checksum"}
        ],
        "CENTER_NAME":                [{"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Parameter", "Column_name": "Center name"}}],
        "RUN_ID":                     [],
        "RUN_DATE":                   [{"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Date", "Column_name": None}}],
        "LANE_NO":                    [{"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Parameter", "Column_name": "Lane number"}}],
        "BASE_COUNT":                 [{"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Comment", "Column_name": "Base count"}}],
        "READ_COUNT":                 [{"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Comment", "Column_name": "Read count"}}],
        "CYCLE_COUNT":                [],
        "SAMPLE_ID":                  [{"f": "meta_to_node", "args": {"Material": "Sample", "Type": "Name", "Column_name": None}}],
        "SAMPLE_NAME":                [{"f": "meta_to_node", "args": {"Material": "Sample", "Type": "Name", "Column_name": None}}],
        "BARCODE":                    [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Barcode sequence"}}],
        "INDEX":                      [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Barcode sequence"}}],
        "SEQUENCING_TYPE":            [{"f": "sequencing_type", "args": None}],
        "INSTRUMENT_MODEL":           [
            {"f": "instrument_model", "args": None},
            {"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Parameter", "Column_name": "Instrument model"}}
        ],
        "INSTRUMENT_PLATFORM":        [{"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Parameter", "Column_name": "Platform"}}],
        "PIPELINE_VERSION":           [],
        "INSERT_SIZE":                [{"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Comment", "Column_name": "Insert size estimate"}}],
        "FRAGMENT_SIZE":              [{"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Comment", "Column_name": "Insert size estimate"}}],
        "LIBRARY_LAYOUT":             [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Library layout"}}],
        "SEQUENCING_READ_TYPE":       [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Library layout"}}],
        "ILSE_NO":                    [{"f": "meta_to_internal", "args": "Batch"}],
        "COMMENT":                    [{"f": "meta_to_internal", "args": "Comment"}],
        "LIB_PREP_KIT":               [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Library kit"}}],
        "INDEX_TYPE":                 [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Barcode kit"}}],
        "ANTIBODY_TARGET":            [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Antibody target"}}],
        "ANTIBODY":                   [{"f": "meta_to_node", "args": {"Protocol": "Library construction", "Type": "Parameter", "Column_name": "Antibody"}}],
        "SEQUENCING_KIT":             [{"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Parameter", "Column_name": "Sequencing kit"}}],
        "PROJECT":                    [{"f": "test_if_unique", "args": False}],
        "BASECALL_SOFTWARE":          [{"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Parameter", "Column_name": "Base caller"}}],
        "BASE_QUAL_ENCODING":         [{"f": "meta_to_node", "args": {"Protocol": "Nucleic acid sequencing", "Type": "Parameter", "Column_name": "Base quality encoding"}}],
        "SPECIES":                    [{"f": "species", "args": None}],
        "%_GC_FROM_FASTQC":           [{"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Comment", "Column_name": "FastQC Per sequence GC content"}}],
        "LANE_%_PASS_PURITY_FILTER":  [],
        "AVERAGE_LANE_Q30":           [],
        "LANE_%_BASE_QUAL_MIN30":     [{"f": "meta_to_node", "args": {"Material": "Assay", "Type": "Comment", "Column_name": "FastQC Per base sequence quality"}}],
        "SAMPLE_SUBMISSION_TYPE":     [],
        "BIOMATERIAL_ID":             [],
        "CUSTOMER_LIBRARY":           [],
        "TAGMENTATION_BASED_LIBRARY": [],
        "PATIENT_ID":                 [{"f": "meta_to_node", "args": {"Material": "Source", "Type": "Name", "Column_name": None}}],
        "GENDER":                     [{"f": "meta_to_node", "args": {"Material": "Source", "Type": "Characteristics", "Column_name": "Sex"}}],
        "SEX":                        [{"f": "meta_to_node", "args": {"Material": "Source", "Type": "Characteristics", "Column_name": "Sex"}}],
    ...
    known_species = {
        "human": {"name": "Homo sapiens", "source": "NCBITAXON", "url": "http://purl.bioontology.org/ontology/NCBITAXON/9606"},
        "mouse": {"name": "Mus musculus", "source": "NCBITAXON", "url": "http://purl.bioontology.org/ontology/NCBITAXON/10090"}
    }

    illumina_instruments = re.compile("(HI|NOVA|NEXT)SEQ *(500|550|1000|1500|2000|2500|3000|4000|6000|X( *Ten)?)", re.IGNORECASE)

    patterns = {
        "Name":            re.compile(".+ Name$"),
        "Characteristics": re.compile("^(Characteristics\\[.+\\]|Term Source REF|Term Accession Number)$"),
        "Parameter":       re.compile("^(Parameter\\[.+\\]|Term Source REF|Term Accession Number)$"),
    ...
    def DkfzMeta(self, fn, species="human", tsv_shortcut="cancer", ignore_undetermined=True):
        ...
        if not "SEQUENCING_TYPE" in object.meta[0].columns:
            raise ValueError("Mandatory column SEQUENCING_TYPE missing from meta file {}".format(fn))
        ...
        for (k, g) in sequencing_types:
            if k in DkfzMetaParser.mappings["SEQUENCING_TYPE_to_Assays"]:
                ...
                for protocol in ["Sample collection", "Nucleic acid extraction", "Library construction", "Nucleic acid sequencing"]:
        ...
        for (assay_type, assay) in object.assays.items():
            for (j, templates) in DkfzMetaParser.column_names.items():
                for template in templates:
                    ...
                    f(object, assay_type=assay_type, meta_column_name=j, args=template["args"])
    ...
    def meta_to_node(self, object, assay_type, meta_column_name, args):
        ...
        if not meta_column_name in meta:
            return
        if not args:
            raise ValueError("No destination for metafile column {}".format(meta_column_name))
        ...
        if "Material" in args:
            ...
            if args["Type"] == "Name":
                ...
                characteristic.set_values(meta[meta_column_name].tolist(), category="values")
                ...
                raise ValueError("Type {} incompatible with material".format(args["Type"]))
        ...
        if "Protocol" in args:
            ...
            if args["Type"] == "Performer" or args["Type"] == "Date":
                protocol.set_values(meta[meta_column_name].tolist(), category=args["Type"])
                ...
                raise ValueError("Type {} incompatible with protocol".format(args["Type"]))
    ...
    def meta_to_path(self, object, assay_type):
        ...
        for i in range(meta.shape[0]):
            m = DkfzMetaParser.patterns["fastq_filename"].match(meta.iloc[i,j_fastq_file])
            if not m:
                logger.error("File {} doesn't match expected pattern".format(meta.iloc[i,j_fastq_file]))
                continue
            p = path / meta.iloc[i,j_run_id] / m.group(1) / "fastq" / meta.iloc[i,j_fastq_file]
            if not p.exists():
    ...
    def test_if_unique(self, object, assay_type, meta_column_name, args):
        ...
        if not unique:
            raise logger.warning("Column {} contains more than one value (all values should be equal)".format(meta_column_name))
    ...
    def instrument_model(self, object, assay_type, meta_column_name, args):
        ...
        for i in range(meta.shape[0]):
            ...
            if match:
                normalized[i] = match.group(1).lower().capitalize() + "Seq " + match.group(2)
    ...
    def species(self, object, assay_type, meta_column_name, args):
        ...
        if object.config["species"] in DkfzMetaParser.known_species:
            ...
            organism.set_default(value=DkfzMetaParser.known_species[object.config["species"]]["name"],   category="values")
            organism.set_default(value=DkfzMetaParser.known_species[object.config["species"]]["source"], category="ref")
            organism.set_default(value=DkfzMetaParser.known_species[object.config["species"]]["url"],    category="accession")
