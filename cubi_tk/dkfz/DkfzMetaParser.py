...
from typing import Dict, List, Tuple, Union, Any, TextIO
...
class DkfzMetaParser:
    ...
    SPECIES = {
        re.compile("([Hh]omo[ _]?[Ss]apiens|hg18|hg19|GRCh37|GRCh38)"): 
            altamisa.isatab.models.OntologyTermRef(
                name="Homo sapiens",
                accession="http://purl.bioontology.org/ontology/NCBITAXON/9606",
                ontology_name="NCBITAXON"
            ),
        re.compile("([Mm]us[ _]?[Mm]usculus|mm9|mm10|GRCm37|GRCm38|GRCm39)"): 
            altamisa.isatab.models.OntologyTermRef(
                name="Mus musculus",
    ...
    def read_meta(self, f: TextIO, ignoreUndetermined=True) -> DkfzMeta:
        ...
        checksums = []
        for row in reader:
            if not "MD5" in row.keys():
                raise MissingValueError("Missing mandatory column \"{}\"".format("MD5"))
            if row["MD5"] in checksums:
                raise DuplicateValueError("MD5 checksum {} appears multiple time".format(row["MD5"]))
            if ignoreUndetermined:
                if "FASTQ_FILE" in row.keys():
            	    if DkfzMetaParser.UNDETERMINED_PATTERN.match(row["FASTQ_FILE"]):
                        continue
                else:
                    logger.warning("Missing column \"{}\", can't test for file of undetermined origin after demultiplexing".format("FASTQ_FILE"))

            if not ("SEQUENCING_TYPE" in row.keys()) or (row["SEQUENCING_TYPE"] is None):
                logger.warning("Missing column \"{}\", can't extract assay type".format("SEQUENCING_TYPE"))
                continue
            if not row["SEQUENCING_TYPE"] in self.schema["Investigation"]["Assays"].keys():
                logger.warning("Unknown assay type \"{}\", data file ignored".format(row["SEQUENCING_TYPE"]))
                continue
            assay_type = row["SEQUENCING_TYPE"]

            checksums.append(row["MD5"])

            if not assay_type in content.keys():
                content[assay_type] = {}
    ...
    def meta_to_isa(self, row: Dict[str, str], assay_type="EXON") -> DkfzMetaRow:
        ...
        processes = DkfzMetaParser._add_library_type(processes, assay_type, protocol_ref="library construction" + " " + assay_type)
    ...
    def _buildMaterial(self, theMaterial: Dict[str, Any], materials: List[altamisa.isatab.models.Material], row: Dict[str, str]) -> altamisa.isatab.models.Material:
        ...
        if ("characteristics" in theMaterial.keys()) and not (theMaterial["characteristics"] is None):
    ...
    def _buildProcess(self, theProcess: Dict[str, Any], processes: List[altamisa.isatab.models.Process], row: Dict[str, str], assay_type: str) -> altamisa.isatab.models.Process:
        ...
        processDate      = datetime.strptime(processDate, "%Y-%m-%d") if not processDate is None else None
    ...
    def _buildArc(self, materials: List[altamisa.isatab.models.Material], processes: List[altamisa.isatab.models.Process], assay_type: str) -> List[DkfzMetaArc]:
        ...
        for arc in self.schema["Assay"]["Arc"]:
            if arc["type"] == "Material":
                ...
                for m in materials:
                    if m.type == arc["name"]:
                        current = DkfzMetaArc(type="Material", name=m.type) # , object=m)
                        break
                if current is None:
                    raise MissingValueError("Missing material {} required in arc".format(arc["name"]))
                ...
                for p in processes:
                    if p.protocol_ref == protocol_ref:
                        current = DkfzMetaArc(type="Process", name=protocol_ref) # , object=p)
                        break
                if current is None:
                    raise MissingValueError("Missing process {} required in arc".format(arc["name"]))
                if not previous is None:
                    arcList.append((previous, current))
                previous = current
            else:
    ...
    def _meta_column_value(self, row: Dict[str, str], theElement: dict, meta_columns: str = "meta_columns") -> Any:
        ...
        if len(theValue) > 1:
            raise DuplicateValueError("Content of {} is not unique: values {} from metafile columns {}".format(theElement.type, ", ".join(theValue), ", ".join(column_names)))
        if len(theValue) == 1:
    ...
    @staticmethod
    def _add_library_type(processes: List[altamisa.isatab.models.Process], assay_type: str, protocol_ref: str = "library construction") -> List[altamisa.isatab.models.Process]:
        ...
        parameterList: Dict[str, altamisa.isatab.models.ParameterValue] = dict([(x.name, x) for x in theProcess.parameter_values])
        for k, v in libraryTypes.items():
            if not k in parameterList.keys():
    ...
    @staticmethod
    def _append_to_Characteristics(characteristics: List[altamisa.isatab.Characteristics], k: str, v: str):
        ...
        for theCharacteristic in characteristics:
            if theCharacteristic.name == k:
                raise DuplicatedValueError("Characteristic {} is not unique".format(theKey))
    ...
    def _append_to_Parameters(parameters: List[altamisa.isatab.ParameterValue], k: str, v: str):
        ...
        for theParameter in parameters:
            if theParameter.name == k:
                raise DuplicatedValueError("Parameter {} is not unique".format(theKey))
    ...
    def _append_to_Comments(comments: List[altamisa.isatab.Comment], k: str, v: str):
        ...
        for theComment in comments:
            if theComment.name == k:
                raise DuplicatedValueError("Comment {} is not unique".format(theKey))
