...
class DkfzPrepareIsatabCommand:
    ...
    def execute(self) -> typing.Optional[int]:
        ...
        if not mapping is None:
            mapping.to_csv(
                self.args.isatab_dir + "/" + "sample_ids_map.txt", sep="\t", index=False
            )
        ...
        for (k, v) in all.assays.items():
            ...
            if filename.exists():
                logger.warning(
                    "ISAtab file {} already exists, not overwritten".format(
                        filename.resolve()
                    )
                )
                continue
            all.get_assay(k).to_csv(
                filename.resolve(), sep="\t", na_rep="", index=False, header=True
            )
