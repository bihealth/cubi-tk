...
from .DkfzExceptions import MissingValueError, IllegalValueError, DuplicateValueError
...
class AssayToFrame:
    ...
    def get_data_frame(self):
        ...
        while current:
            if current in self.assay.materials.keys():
                ...
                raise IllevalValueError("Unknown element {}".format(current))
    ...
    def _add_material_columns(self, current):
        ...
        if currentType in self.attributes.keys():
            raise DuplicateValueError("Assay graph not DAG: type {} appears multiple times in path".format(currentType))
    ...
    def _assay_row(self, raw_data_file):
        ...
        while current:
            ...
            if current in materials.keys():
                m = materials[current]
                currentType = m.type
                AssayToFrame._insert_at_start(row, AssayToFrame._add_comments(self.attributes[currentType]["comments"], m, raw_data_file))
                AssayToFrame._insert_at_start(row, AssayToFrame._add_factors(self.attributes[currentType]["factors"], m, raw_data_file))
                AssayToFrame._insert_at_start(row, AssayToFrame._add_characteristics(self.attributes[currentType]["characteristics"], m, raw_data_file))
                row.insert(0, m.name)
                ...
                AssayToFrame._insert_at_start(row, AssayToFrame._add_comments(self.attributes[currentType]["comments"], p, raw_data_file))
                AssayToFrame._insert_at_start(row, AssayToFrame._add_parameters(self.attributes[currentType]["parameters"], p, raw_data_file))
            ...
            if currentType is None or i >= len(self.path) or currentType != self.path[i]:
                print("Incompatible paths, {} not added to assay ISATAB table".format(raw_data_file))
                continue
            current = self.parent[current] if current in self.parent.keys() else None
            i += 1
