...
def _recreate_md5_for(path: Path):
    ...
    """
    try:
        logger.debug("Calling `md5sum {}`", path)
        md5_out = subprocess.check_output(["md5sum", str(path)])
    except subprocess.SubprocessError as e:
        logger.info("Problem with MD5 computation: {}", e)
        return False

    md5_str = md5_out.decode("utf-8").strip().split()[0]
    logger.debug("MD5 sum of {} is {}", path.name, md5_str)

    try:
        with Path("%s.md5" % path).open("wt") as md5_file:
            logger.info("Writing MD5 sum to {}", md5_file.name)
            print("%s  %s" % (md5_str, path.name), file=md5_file)
    except IOError as e:
        logger.error("Problem writing MD5 sum: {}", e)
        return False
    else:
        return True
...
def _call(cmd: typing.List[str], cwd=None):
    ...
    try:
        ...
        tpl = (
            "md5sum --check call did not succeed: {}\n--- stdout BEGIN ---\n{}--- stdout END ---\n"
            "--- stderr BEGIN ---\n{}--- stderr END ---"
        )
        logger.error(
            tpl,
            e,
            "{}\n".format( e.stdout.decode("utf-8") if e.stdout else ""),
            "{}\n".format(e.stderr.decode("utf-8") if e.stderr else ""),
        )
        return False
    else:
        return True
...
def run_check(ok: Value, path: Path, args):
    ...
    if not _check_gz_integrity(path):
        logger.error("GZip file integrity check failed: {}", path)
        with ok.get_lock():
            ok.value = False
        ...
        logger.debug("MD5 file exists: {}", path_md5)
        if _check_md5(path):
            logger.debug("  => MD5 OK")
        else:
            logger.error("  => MD5 mismatch for {}", path)
            with ok.get_lock():
                ok.value = False
    elif args.missing_md5_error:  # => .md5 missing and is error => error
        logger.error("MD5 file does not exist for: {}", path)
        with ok.get_lock():
            ok.value = False
    elif args.compute_md5:  # .md5 missing and is not error => recreate
        recreated = _recreate_md5_for(path)
        if recreated:
            logger.info("Created MD5 file: {}", path_md5)
        elif args.create_md5_fail_error:
            logger.error("Could not create MD5 file for: {}", path)
            with ok.get_lock():
                ok.value = False
    else:
        logger.info("Not attempting to recreate {}", path_md5)
...
class CheckCommand:
    ...
    def _run_loop(self) -> int:
        ...
        logger.info(
            "Checking {} files with a total size of {}",
            len(self.args.paths),
