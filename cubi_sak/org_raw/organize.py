"""``cubi-sak org-raw organize``: organization of FASTQ files"""
...
def _organize(ok: Value, counter: Value, job: OrganizeJob, args, t: tqdm.tqdm):
    logger.debug("%s %s to %s", "Moving" if args.move else "Copying", job.src, job.dest)
    if not job.dest.parent.exists():
        logger.debug("Creating directory %s", job.dest.parent)
...
class OrganizeCommand:
    ...
    def execute(self) -> typing.Optional[int]:
        ...
        logger.info("Starting cubi-sak org-raw organize")
        ...
        for job in jobs:
            logger.info(
                ...
                job.src,
                job.dest,
                " (+.md5)" if Path("%s.md5" % job.src).exists() else "",
            )
...
def setup_argparse(parser: argparse.ArgumentParser) -> None:
    """Setup argument parser for ``cubi-sak org-raw check``."""
